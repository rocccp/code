...
myLib.desktop.lrBar = {
	...
	init : function(iconData) {
		...
		$default_app
		/*
		 * .droppable({ scope:'a', drop: function(event,ui) { var
		 * title=ui.draggable.find(".text").text(); ui.draggable
		 * .removeClass("desktop_icon") .attr({"style":"", "title":title })
		 * .find("span") .removeClass("icon") .end() .appendTo($default_app);
		 * myLib.desktop.deskIcon.init(); _this.init(); } })
		 */
		...
		/*
		 * .draggable({ helper: "clone", scroll:false, opacity: 0.7, scope:'a',
		 * appendTo:'parent', start:function(){ $lrBar.css("z-index",90); } })
		 * .droppable({ scope:'a', drop: function(event,ui) {
		 * 
		 * var title=ui.draggable.find(".text").text(); ui.draggable
		 * .removeClass("desktop_icon") .attr({"style":"", "title":title })
		 * .find("span") .removeClass("icon") .end() .insertBefore($(this));
		 * _this.init(); myLib.desktop.deskIcon.init(); } });
		 */
		// 开始按钮、菜单交互效果
		...
		$("#weather_btn").click(function() {
			...
			myData.panel.taskBar['task_lb'].find('a').each(function(index) {
				var id = "#myWin_" + $(this).data("win");
				myLib.desktop.win.minimize($(id));
			});
		});
...
myLib.desktop.deskIcon = {
	...
	upDesktop : function($deskIcon, $deskIconBlock, $innerPanel, $deskIconNum,
		...
		var myData = myLib.desktop.getMydata(), winWh = myData.winWh, w = winWh['w']
	...
	init : function(iconData) {
		...
		var myData = myLib.desktop.getMydata(), winWh = myData.winWh, $deskIconBlock = myData.panel.desktopPanel['_this'], $innerPanel = myData.panel.desktopPanel.innerPanel, $deskIcon = myData.panel.desktopPanel['deskIcon'], $deskIconNum = $deskIcon

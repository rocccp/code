	...
	$.addFlex = function(t, p) {
		...
		var g = {
							...
							.each(
									...
									function(i, row) {
										...
										if (p.checkbox) {
										    ...
										    //zhaojp
											if (checked) {
												$(cthch).attr("checked","true");
												objTr.addClass('trSelected');
											}

											// checkedBox[0].checked = checked;
											// alert(checkedBox[0].checked);
			...
			populate : function() { // get latest data
				...
				var queryFilter = "";
				var queryFilterLength = $('#queryFilter').length;
				if (queryFilterLength > 0) {
		...
		if (p.usepager) {
			...
			// title="首页"><span></span></div><div class="pPrev pButton"
			// title="上一页"><span></span></div> </div> <div
			// class="btnseparator"></div> <div class="pGroup"><span
			// class="pcontrol">第 <input type="text" size="4" value="1" />页,共
			// <span> 1 </span>页</span></div> <div class="btnseparator"></div>
			// <div class="pGroup"> <div class="pNext pButton"
			// title="下一页"><span></span></div><div class="pLast pButton"
			...
			if (p.searchitems) {
				...
				$(g.sDiv)
						.append(
								"<div class='sDiv2'>快速查找 <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"
										+ sopt
										+ "</select> <input type='button' value='清除' /></div>");
